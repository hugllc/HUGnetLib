...
if (!class_exists("dbsocket")) {
    ...
    *
    * This class is meant to talk to something like
    * {@link http://ser2net.sourceforge.net/ ser2net}
    * or any of the serial to ethernet servers from
    * {@link http://www.bb-elec.com/ B&B Electronics}.
    ...
    */
    class DbSocket extends SocketBase
    {
        ...
        * @param bool  $GetReply Whether or not to get a reply
        *
        * @return mixed
        */
        public function write($pkt, $GetReply = true)
        {
            $id                      = rand(1, 24777216);  // Big random number
            $pkt["PacketFrom"]       = $pkt["SentFrom"];
            $this->packet[$id]       = $pkt;
            $this->packet[$id]["id"] = $id;
            if ($GetReply) {
                $this->replyId[$id] = time();
            }
        ...
        */
        protected function replyIdCleaner()
        {
            foreach ($this->replyId as $id => $time) {
                if ($time < (time() - $this->ReplyTimeout)) {
                    unset($this->replyId[$id]);
                }
            }
        }
        /**
        ...
        */
        function unbuildPacket($data)
        {
            ...
            if (empty($data["Command"])) {
                $pkt["Command"] = $data["sendCommand"];
            } else {
                $pkt["Command"] = $data["Command"];
            }
            if (empty($data["PacketTo"])) {
                $pkt["To"] = $data["To"];
            } else {
        ...
        *
        * This routine only checks the connection.  It does nothing else.  If
        * you want to have the script automatically connect if it is not
        * connected already then use dbsocket::Connect().
        *
        ...
        *
        * This function first checks the connection.  If you are planning on
        * checking the connection and want the server to automatically connect
        * if not connectd, use this routine.  If you just want to check the
        * connection, use ep_socket::CheckConnect.
        *
