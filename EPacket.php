...
class EPacket extends HUGnetClass
{
    ...
    */
    ...
    {
        if ($this->verbose > 1) {
            print "Checking for Callback Function...  ";
        }
        ...
        if (is_object($this->callBackObject)) {
            if ($this->verbose > 1) {
                print " ".get_class($this->callBackObject)." ";
            }
            if (method_exists($this->callBackObject, $function)) {
                if ($this->verbose > 1) {
                    $class = get_class($this->callBackObject);
    ...
    */
    ...
    {
        if ($this->verbose) {
            print "Setting up the packet send\n";
        }
    ...
    */
    function sendPacket(&$Info, $PacketList, $GetReply=true, $pktTimeout = null)
    {
        ...
        if ($this->verbose) {
            print "Sending a packet on ";
            print $this->Info[$socket]["GatewayIP"];
            print ":".$this->Info[$socket]["GatewayPort"]."\n";
        }
        ...
        foreach ($PacketList as $Packet) {
            ...
            if (!is_object($this->socket[$socket])) {
            ...
            if (is_array($retval)) {
                $ret = array_merge($ret, $retval);
            }
            unset($this->Packets[$index]);
        }

        if (($retval === true) && empty($ret)) {
            return true;
        }
        if (empty($ret)) {
            return false;
    ...
    */
    ...
    {
        $ret = array();
        if ($this->Packets[$index]["GetReply"] === false) {
            return true;
        }
        do {
    ...
    * @return bool
    */
    private function _checkSN($Info, $key)
    {
        $Info = array("DeviceID" => $this->SNArray[$key]);
        if ($this->verbose) {
            print "Checking Serial Number ".$this->SNArray[$key]."\r\n";
        }
    ...
    */
    ...
    {
        return (($pkt["SentTo"] == $reply["From"])
                || $pkt['group']) && ($pkt["SentFrom"] == $reply["To"]);
    }
    ...
    */
    ...
    {
        if ($reply['Command'] != PACKET_COMMAND_REPLY) {
            return false;
        }
    ...
    */
    ...
    {
        if ($pkt["Reply"] !== true) {
            $pkt["To"]          = devInfo::setStringSize($pkt["To"], 6);
            $pkt["Unsolicited"] = ($pkt["To"] == $this->unsolicitedID);
            $pkt["Socket"]      = $socket;
            $pkt["GatewayKey"]  = $socket;
            $pkt["Reply"]       = false;
            $pkt["toMe"]        = ($pkt["To"] == $this->SN);
    ...
    *
    * @param string $pkt  The packet we got
    ...
    */
    function interpPacket($pkt, $sock=0)
    {
        ...
        if ($pkt["Checksum"] != $pkt["CalcChecksum"]) {
            if ($this->verbose) {
                print "Bad Packet Received: ".$pkt["RawData"]."\r\n";
            }
            return false;
        }
    ...
    */
    function monitor($config=array(), $timeout = 0)
    {
        ...
        $this->getAll(true);
        if ($this->verbose) {
            print "Waiting on Packets from ";
            print $config["GatewayName"].":".$config["GatewayPort"]."\n";
        }
    ...
    */
    ...
    /**
    * Check if we are connected.  If not it connects to the gateway specified
    * in $config.
    *
    ...
    */
    function connect($config)
    {
        ...
        if ($this->checkConnect($config["GatewayKey"])) {
            return true;
        }
        $ret = $this->_connectOpenSocket($config);
        if ($ret) {
            $ret = $this->socket[$config["GatewayKey"]]->connect();
        }
        if ($ret) {
            $this->_connectSetSN($config);
        }
        return true;
    }
    ...
    *
    * @param array $config The configuration information to use
    *
    * @return bool false on failure, true on success
    */
    ...
    */
    ...
    /**
    * If $config is given it will try to connect to the server that
    * is specified in it.
    *
    ...
    */
    ...
    {
        if (!isset($config["packetSNCheck"])) {
            $config["packetSNCheck"] = $snCheck;
        }
        ...
        if (empty($config["GatewayKey"])) {
            $config["GatewayKey"] = 1;
        }
        if (!empty($config["ReplyTimeout"])) {
            $this->ReplyTimeout = $config["ReplyTimeout"];
        }
