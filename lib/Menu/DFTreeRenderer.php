<?php
/**
	@file Tree/DFTreeRenderer.php

		
	
	@par Original Copyright
	This file was originally pear/HTML/Menu/DirectTreeRenderer.php
	@verbatim
//
// +----------------------------------------------------------------------+
// | PHP Version 4                                                        |
 ...
 * @package HTML_Menu
 */
class HTML_Menu_DFTreeRenderer extends HTML_Menu_Renderer
{
   ...
   /**
    * Values that should always be present
    * @see renderEntry()
    * @var array
    */
    var $_expectedReplace = array('popup' => '', 'class' => '', 'extra' => '');
    ...
    function renderEntry($node, $level, $type)
    {
        ...
        if (!isset($node['help']) || !$this->_useHelp) $node['help'] = '';
        $keys = $values = array();
        $node = array_merge($this->_expectedReplace, $node);
        foreach ($node as $k => $v) {
            if ('sub' != $k) {
                $keys[]   = '{' . $k . '}';
                $values[] = $v;
        ...
        $useLevel = (isset($this->_entryTemplates[$level][$type])) ? $level : 'default';
        $this->_itemHtml[$level] = str_replace($keys, $values, $this->_entryTemplates[$useLevel][$type]);
    }
    ...
    */
    function setEntryTemplate($type, $template = null, $level='default')
    {
        if (is_array($type)) {
            // array_merge() will not work here: the keys are numeric
            foreach ($type as $typeId => $typeTemplate) {
                if (isset($this->_entryTemplates['default'][$typeId])) {
                    $this->_entryTemplates[$level][$typeId] = $typeTemplate;
                }
            }
        } else {
            $this->_entryTemplates[$level][$type] = $template;
        }
    }
