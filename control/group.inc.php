...
$gForm->addElement(
    'text', 'DeviceGroup', "Group:", array('size' => 6, 'maxlength' => 6)
);
$gForm->addElement(
    'text', 'BoredomThreshold', "Boredom:", array('size' => 3, 'maxlength' => 3)
);
$gForm->addRule(
    'DeviceGroup', 'Group can not be empty', 'required', null, 'client'
);
$gForm->addRule(
    'DeviceGroup', 'Group must be alphanumeric', 'alphanumeric', null, 'client'
);
$gForm->addRule(
    'BoredomThreshold', 'Boredom can not be empty', 'required', null, 'client'
);
$gForm->addRule(
    'BoredomThreshold', 'Boredom must be numeric', 'numeric', null, 'client'
);
...
if (isset($_REQUEST['postGroup']) && $gForm->validate()) {
    ...
    if ($return) {
        header(
            "Location: ".$_SERVER['PHP_SELF']."?DeviceKey=".$devInfo['DeviceKey']
        );
    }
}
print $gForm->toHTML();
