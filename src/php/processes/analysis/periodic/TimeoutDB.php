 ...
 * @since      0.9.7
 */
class TimeoutDB extends \HUGnet\processes\analysis\Periodic
{
    /** This is the period */
    ...
    */
    ...
    {
        if ($this->ready()) {
            $this->system()->out("Checking for long running select queries");
            $db = $this->system()->table("Generic");
            $ret = $db->selectTimeout();
            $this->system()->out("Killed $ret queries");
            $this->success();
        }
    }
}
