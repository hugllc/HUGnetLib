 ...
 */
class OmegaAlphaPH extends \HUGnet\devices\inputTable\DriverADuC
    implements \HUGnet\devices\inputTable\DriverInterface
{
    ...
    */
    protected $params = array(
        "longName" => "Omega Alpha pH sensor",
        "shortName" => "OmegaAlphaPH",
        "unitType" => "pH",
        "storageUnit" => 'pH',
        ...
        "extraDesc" => array(
            ...
            "The voltage when the sensor reads 0 pH",
    ...
    */
    protected function getReading(
        ...
        $Vmin  = $this->getExtra(4);

        $A = $this->inputBiasCompensation($A, $Rin, $Rbias);
        $Va = bcmul(bcdiv($A, $Am), $Vref);
        $pH = $this->linearBounded($Va, $Vmin, $Vmax, 14, 0);

        return round($pH, $this->get('maxDecimals'));
    }
    /**
    ...
    */
    protected function getRaw(
        ...
        $Rbias = $this->getExtra(1);
        $Vref  = $this->getExtra(2);
        $Vmin  = $this->getExtra(3);
        $Vmax  = $this->getExtra(4);

        $Va = $this->linearBounded($value, 0, 14, $Vmin, $Vmax);
        $A = ($Va / $Vref) * $Am;
